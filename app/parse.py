...
def get_single_course_info(course_common_info: BeautifulSoup, time_type: CourseType) -> Course:
    ...
    if time_type.value == time_type.FULL_TIME.value:
        course.duration = int(detailed_page_soup.select("p.CourseModulesHeading_text__EdrEk")[2].text.split()[0])
    return course
